...
export class ActivitiesPartnerService {
  constructor(
    @InjectRepository(AdventureActivity)
    private readonly activityRepo: Repository<AdventureActivity>,
    @InjectRepository(AdventureActivityMedia)
    private readonly mediaRepo: Repository<AdventureActivityMedia>,
    @InjectRepository(AdventureActivityRequirements)
    private readonly reqsRepo: Repository<AdventureActivityRequirements>,
    @InjectRepository(AdventureActivityInclusion)
    private readonly inclusionRepo: Repository<AdventureActivityInclusion>,
  ...
  async uploadMedia(file: Express.Multer.File, isCover: boolean) {
    const url = `https://storage.niklo.com/adventure/activities/${file.originalname}`;
